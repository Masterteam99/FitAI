      ...
      <section className="max-w-[1180px] mx-auto px-7 pt-20 pb-10 text-center">
        <FadeIn>
          <span className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.22em] font-semibold mb-5" style={{ color: "var(--organic-green-deep)" }}>
            ...
            <EditableText path="perChi.badge">{c.badge}</EditableText>
          </span>
        </FadeIn>
        <SlideUp delay={0.05}>
          <h1 className="text-display-lg !text-[clamp(2.6rem,5vw,4rem)] mb-5">
            <EditableText path="perChi.heroTitle.pre">{c.heroTitle.pre}</EditableText>
            <em style={{ color: "var(--organic-green)" }}><EditableText path="perChi.heroTitle.highlight">{c.heroTitle.highlight}</EditableText></em>
          </h1>
        </SlideUp>
        <FadeIn delay={0.15}>
          <p className="text-lg text-muted-foreground max-w-[620px] mx-auto"><EditableText path="perChi.heroSubtitle">{c.heroSubtitle}</EditableText></p>
        </FadeIn>
      </section>
            ...
            return (
              <StaggerItem key={s.title}>
                <div className="group bg-card border border-border rounded-[22px] p-8 h-full transition-all duration-300 hover:-translate-y-1.5 hover:border-[var(--organic-terracotta-soft)] hover:shadow-[0_24px_50px_-28px_rgba(0,0,0,.4)]">
                  ...
                  >
                    <EditableText path="perChi.segmentCta">{c.segmentCta}</EditableText> <ChevronRight className="w-4 h-4" />
                  </Link>
                </div>
              </StaggerItem>
            );
