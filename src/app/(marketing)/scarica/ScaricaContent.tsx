...
export function ScaricaContent() {
  const copy = useCopy();
      ...
      <section className="max-w-3xl mx-auto px-4 py-20 text-center space-y-5">
        <SlideUp>
          <h1 className="text-display-lg">
            <EditableText path="scarica.heroTitle.pre">{c.heroTitle.pre}</EditableText>
            <span className="text-gradient-energy"><EditableText path="scarica.heroTitle.highlight">{c.heroTitle.highlight}</EditableText></span>
          </h1>
        </SlideUp>
        <FadeIn delay={0.12}>
          <p className="text-xl text-muted-foreground"><EditableText path="scarica.heroSubtitle">{c.heroSubtitle}</EditableText></p>
        </FadeIn>
        <FadeIn delay={0.2}>
          <p className="inline-flex items-center gap-2 text-sm px-4 py-2 rounded-full" style={{ background: "rgba(15,158,153,.10)", color: "var(--organic-green-deep)" }}>
            <EditableText path="scarica.note">{c.note}</EditableText>
          </p>
        </FadeIn>
        <FadeIn delay={0.28}>
      ...
      <section className="max-w-3xl mx-auto px-4 pb-24">
        <div className="rounded-[22px] p-7 md:p-8" style={{ background: "var(--organic-espresso)", color: "var(--foreground)" }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-display text-lg mb-2"><EditableText path="scarica.whyBrowser.title">{c.whyBrowser.title}</EditableText></h2>
              <p className="text-sm" style={{ color: "rgba(234,241,248,.8)" }}><EditableText path="scarica.whyBrowser.body">{c.whyBrowser.body}</EditableText></p>
            </div>
          </div>
        </div>
      </section>
