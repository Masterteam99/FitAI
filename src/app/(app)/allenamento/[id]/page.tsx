    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {plan.isActive && <Badge className="bg-primary/20 text-primary border-primary/30">Attivo</Badge>}
            {plan.generatedByAI && <Badge variant="secondary" className="gap-1"><Zap className="w-3 h-3" />AI</Badge>}
          </div>
          <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground flex-wrap">
            <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" />{plan.durationWeeks} settimane</span>
            <span className="flex items-center gap-1.5"><Dumbbell className="w-4 h-4" />{plan.workoutsPerWeek}x/settimana</span>
            <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" />~{estimatedMinutes} min/sessione</span>
          </div>
        </div>
      </div>
          ...
          <Card key={day.id} className={day.restDay ? "opacity-60" : ""}>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">Giorno {day.dayNumber}: {day.name}</CardTitle>
            ...
            <CardContent>
              {day.restDay ? (
                <p className="text-sm text-muted-foreground">Giorno di riposo — recupero attivo consigliato</p>
              ) : day.exercises.length === 0 ? (
                <p className="text-sm text-muted-foreground">Nessun esercizio aggiunto</p>
              ) : (
      ...
      {plan.days.some((d) => !d.restDay && d.exercises.length > 0) && (
        <div className="flex items-center gap-3 pt-2 p-4 rounded-xl bg-primary/5 border border-primary/20">
          ...
          <p className="text-sm">Seleziona il giorno che vuoi allenare e premi <strong>Inizia</strong> per avviare la sessione guidata.</p>
        </div>
      )}
    </div>
  );
