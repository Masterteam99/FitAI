...
export const metadata: Metadata = { title: copy.analisi.meta.title };

const SYSTEM_COLORS = ["text-blue-400", "text-primary", "text-purple-400"];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {copy.analisi.subtitle}
        </p>
      </div>

      {/* Come funziona */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {copy.analisi.systems.map((item, i) => (
          <div key={item.title} className="bg-card border border-border rounded-xl p-4 space-y-2">
            <div className={`text-2xl font-bold ${SYSTEM_COLORS[i]}`}>{item.pct}</div>
      ...
      <div>
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          ...
          {copy.analisi.listTitle(exercises.length)}
        </h2>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Badge variant="secondary" className="text-primary text-xs">{copy.analisi.aiReadyBadge}</Badge>
