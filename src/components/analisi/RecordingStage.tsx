...
import { Brain, Camera, Loader2, SwitchCamera, Square } from "lucide-react";
...
export function RecordingStage({
  ...
  // In RECORDING la fotocamera diventa protagonista a schermo (l'utente deve
  // vedersi bene mentre esegue): il video di riferimento del PT resta
  // disponibile in un riquadro (PIP) in alto a destra, solo da desktop in su
  // — su mobile lo spazio non basterebbe. In IDLE/COUNTDOWN resta l'affiancato
  // normale, utile per prepararsi guardando il PT prima di partire.
  const isRecording = phase === "RECORDING";

  return (
    <div className={isRecording ? "space-y-4" : "space-y-4 max-w-4xl"}>
        ...
        <Card className="overflow-hidden">
          <CardContent className="p-0 relative">
            <div className="relative bg-black" style={{ aspectRatio: "16/10" }}>
              ...
              <div className="hidden lg:block absolute top-4 right-4 w-52 rounded-xl overflow-hidden border-2 border-background shadow-2xl">
                {refPanel}
              </div>
            </div>
            {exercise && (
          ...
          <Card className="overflow-hidden">
            <CardContent className="p-0 relative">
              <div className="aspect-video bg-black relative">
                ...
                {stream && canSwitchCamera && onSwitchCamera && (
                  <button
                    type="button"
      ...
      <div className="flex justify-center">
        ...
        {isRecording && onEndEarly && (
          <Button size="lg" variant="outline" onClick={onEndEarly} className="gap-2 px-8">
            <Square className="w-4 h-4" />
            {copy.analisiSessione.endEarly}
          </Button>
        )}
      </div>
    </div>
